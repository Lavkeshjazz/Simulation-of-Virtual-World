class Polygon {
    //Around the points a polygon will be created
       constructor(points) {
          this.points = points;
          this.segments = [];
          for (let i = 1; i <= points.length; i++) {
             this.segments.push(
                new Segments(points[i - 1], points[i % points.length])
             );
          }
       }
       ...
       static break(poly1, poly2) {
          ...
          for (let i = 0; i < segs1.length; i++) {
            ...
            //This means the points does not intersect at the tip
                if (int && int.offset != 1 && int.offset != 0) {
                   const point = new Point(int.x, int.y);
                   let aux = segs1[i].p2;
                   segs1[i].p2 = point;
                   segs1.splice(i + 1, 0, new Segments(point, aux));
                   aux = segs2[j].p2;
                   segs2[j].p2 = point;
                   segs2.splice(j + 1, 0, new Segments(point, aux));
                }
             }
          }
       }
